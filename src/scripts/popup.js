		...
		if (!data?.details?.user?.name) {
			...
			chrome.storage.sync.get(['loggedIn'], data => {
				...
				if (!data.loggedIn) {
					...
					port.postMessage({
						type: 'login',
						details: { name: userData.details.user.name }
					});
				}
			});
		}
	} else if (data.type === 'error.auth') {
		handleAuthError(data.details.message);
	}
});

function handleAuthError(error) {
	const authErrorContainer = document.getElementById('auth-error-feedback');
	// authErrorContainer.innerText = error;
	// authErrorContainer.style.opacity = '1';
